...
The above diagram shows the high-level deployment architecture of the WIChat application, divided into Client Node (web contents of the application) and Application Server (Deployed modules in the virtual machine of Azure). Each deployment block will be containerized in Docker packages to grant several quality features. The system of the application is divided into three main components: the **frontend**, the **backend**, and the **external services** (LLM API and Wikidata). This last component of external servicies does not have to be deployed, since it consists of external services that are accessed via HTTPS.
